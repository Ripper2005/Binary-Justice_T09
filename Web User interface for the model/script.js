...
const API_ENDPOINT = 'https://<your-backend-url>/predict';

// Backend status endpoint for health checks
const STATUS_ENDPOINT = 'https://<your-backend-url>/status';
...
async function checkBackendStatus() {
    try {
        ...
        if (response.ok) {
            ...
            statusIndicator.title = 'Backend service is operational';
        } else {
            throw new Error('Service unavailable');
        }
    } catch (error) {
        statusIndicator.className = 'status-offline';
        statusIndicator.querySelector('.status-text').textContent = 'Service Offline';
        statusIndicator.title = 'Backend service is currently unavailable';
    }
}
